...
export async function initializeDatabase(pool: Pool): Promise<void> {
  console.log("\n🌟🗄️  [DB INIT] Let's get your database ready!");
  ...
  try {
    ...
    if (sqlFilePath) {
      console.log(
        `📜  Using SQL file: \x1b[36m${path.basename(sqlFilePath)}\x1b[0m`
      );
      sqlContent = fs.readFileSync(sqlFilePath, "utf8");
    } else {
      console.log("⚡️  No SQL file found, using fallback schema.");
      sqlContent = getFallbackSchema();
    }

    const statements = splitSqlStatements(sqlContent);
    console.log(
      `🧩  Preparing to execute \x1b[33m${statements.length}\x1b[0m SQL statements...`
    );
    ...
    try {
      ...
      console.log(
        `🎊✨  Database schema initialized: ${completed}/${statements.length} statements executed successfully! 🚀\n`
      );
    } catch (error) {
